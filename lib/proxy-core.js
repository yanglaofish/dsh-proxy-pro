...
function dualBadge(probe) {
  // Keep the HTTP code in the copy (2026-09-18 用户要求：展示原因时直接看到
  // 状态码，而不是只有人话）。连接层失败没有 status，保持原文案。
  const detail = (s) => String(s ?? '')
  if (!probe) return { text: '结果未知', tone: 'warn' }
  if (probe.verdict === 'usable') {
    return { text: Number.isFinite(probe.status) ? `链路通 · 目标正常响应（HTTP ${probe.status}）` : '链路通 · 目标正常响应', tone: 'ok' }
  }
  ...
  if (probe.kind === 'gateway' || probe.kind === 'server-error') {
    return { text: `链路通 · 目标/网关报错（${detail(probe.short) || `HTTP ${probe.status ?? '5xx'}`}）`, tone: 'warn' }
  }
...
export function combineDualProbe(proxyProbe, directProbe) {
  ...
  const notReachable = (probe, label) => {
    // Keep the HTTP code — the reason line should name the status
    // (2026-09-18: user asked for the code to be visible where the cause is shown).
    const detail = probe?.short ?? '未知原因'
    return `${label}：${detail}`
  }
