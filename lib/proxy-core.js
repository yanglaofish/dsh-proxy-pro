 ...
 * ProxyOverride is a direct-connection whitelist and is merged into NO_PROXY
 * in EVERY mode (system/custom), not only mode 'system': bypass decisions are
 * orthogonal to where the proxy address comes from (§17.3f).
...
export function resolveProxyState(config, systemProxy = null) {
  const noProxy = composeNoProxy(config?.noProxy, systemProxy?.override)
