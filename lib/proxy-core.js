...
/**
 * Turn a fetch rejection into a short, actionable verdict for the panel and the
 * `proxy_test` tool.
 *
 * A bare `fetch failed` hides the real cause. The host runs TWO undici copies —
 * Node's built-in `fetch` and the app's `undici` package — and a dispatcher
 * built by one handed to the other rejects with UND_ERR_INVALID_ARG ("invalid
 * onRequestStart method"); that is exactly how the forced-channel diagnostic
 * failed (LESSONS §22). Surface the cause code so the next report names it.
 *
 * @param error - the rejection from `fetch`.
 * @returns `{ ok: false, kind, short }` with a cause-aware message.
 */
export function classifyTargetFailure(error) {
  ...
  if (code === 'UND_ERR_INVALID_ARG' || /invalid onRequestStart/i.test(String(cause?.message ?? ''))) {
    return { ok: false, kind: 'dispatcher', short: `dispatcher rejected by this undici instance (${code || 'UND_ERR_INVALID_ARG'}) — mixed undici copies` }
  }
  ...
  return { ok: false, kind: 'other', short: (detail ? `${base} (${detail})` : base).slice(0, 180) }
}
