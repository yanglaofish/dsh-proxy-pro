...
/**
 * One probe attempt: HEAD first, then a GET retry when the endpoint does not
 * implement HEAD (405/501) — many APIs only answer GET, and a HEAD-only probe
 * would libel a perfectly usable host. `request` is the caller's fetch: the
 * global one for the policy channel, this undici copy's `fetch` for forced
 * channels (LESSONS §22 — the two must never be mixed).
 *
 * @returns a verdict from classifyProbeStatus / classifyTargetFailure.
 */
async function probeHttp(request, target, dispatcher) {
  ...
  retried.note = retried.ok === true
    ? 'HEAD was rejected but GET works — the host is usable.'
    : 'HEAD was rejected; the GET retry hit the same class of problem.'
  return retried
}
...
async function forceChannelProbe(url, channel, proxyUrl) {
  ...
  if (und === null) {
    return { ok: false, verdict: 'unusable', kind: 'unavailable', short: 'forced channel unavailable (undici could not be loaded)',
      why: 'The app-side undici package could not be imported by the host process.',
      fix: 'Uncheck 使用代理 to use the policy channel, or restart DSH; report the plugin version if it persists.' }
  }
  let dispatcher
  try {
    ...
    return { ok: false, verdict: 'unusable', kind: 'bad-proxy', short: `invalid proxy address: ${String(error?.message ?? error)}`,
      why: 'The effective proxy address is not a valid http(s) proxy URL.',
      fix: 'Fix the address in 代理配置 (mode = 自定义地址) or clear the custom URL.' }
  }
  try {
 ...
 * real request through the installed policy (global fetch → policy dispatcher).
 * `channel` selects the probe path: 'policy' (default), 'proxy' (forced via
 * proxy), or 'direct' (forced bypass).
 */
async function probeTarget(url, snapshot, channel = 'policy') {
  ...
  try {
    ...
    return { url, route: 'invalid', bypassed: false,
      probe: { ok: false, verdict: 'unusable', kind: 'invalid', short: 'invalid URL — expected http(s)://host/path',
        why: 'The text is not a parseable absolute URL.', fix: 'Use the full form, e.g. https://github.com/foo/bar.' } }
  }
  ...
  if (channel === 'proxy' || channel === 'direct') {
    ...
    const probe = proxyUrl === '' && channel === 'proxy'
      ? { ok: false, verdict: 'unusable', kind: 'bad-proxy', short: 'no active proxy url in the current snapshot',
        why: 'The proxy switch is off (or the Windows system proxy is off), so there is nothing to force traffic through.',
        fix: 'Turn the proxy on, or fix the address/mode in 代理配置.' }
      : await forceChannelProbe(parsed, channel, proxyUrl)
    return { url, route: routeName, bypassed, channel, probe }
  }
  const probe = await probeHttp((u, i) => fetch(u, i), parsed, undefined)
    ...
    output: {
      ...
      render: (_args, value) => {
        ...
        const head = p.verdict === 'usable'
          ? `usable${p.status === undefined ? '' : ` (HTTP ${p.status})`}`
          : `${p.verdict === 'degraded' ? 'degraded' : 'unusable'} — ${p.short ?? 'unknown'}`
        const lines = [
          `Route: ${value.route}`,
