...
function apply(ctx, config) {
  ...
  const sync = async () => {
    ...
    // Read system facts in every enabled mode: mode 'system' needs the proxy
    // address, mode 'custom' still needs the ProxyOverride whitelist merged
    // into NO_PROXY (§17.3f).
  ...
  // Follow the Windows system proxy at runtime (change-only re-apply). The
  // system ProxyOverride feeds NO_PROXY in custom mode too, so any enabled
  // mode re-syncs when system facts change (§17.3f).
