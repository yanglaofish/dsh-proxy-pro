...
/** Settings namespace owning the proxy configuration (shared with stock
 *  tooling). Plain literal: the legacy settingsNamespace() helper was removed
 *  from newer dsh-settings, and 'proxy' is already a valid lowercase-hyphenated
 ...
 * from this (env.get must return `{ value }` or undefined). Only the lower-case
 * spellings are fed in — the resolver reads the lower-case name first, and the
 * library's applyPolicyEnv publishes a single spelling on Windows, so a
 * dual-cased input is just a duplicate echo of the very bug this plugin
 * inherited from dsh-http-proxy.
 */
...
/**
 * 一个探测尝试：先 HEAD，HEAD 结论不可信（404/405/501）时再用 GET 复测——
 * 405/501 是「不支持该方法」，404 是网关不给 HEAD 注册路由时的伪装
 * （LESSONS §27：DashScope `HEAD /models` → 404，同路径 GET → 401），只用
 * HEAD 会把健康端点误判成「路径不存在」。`request` 是调用方的 fetch：两条
 * 通道都用 app undici 的 `und.fetch` 配同副本 dispatcher（LESSONS §22/§25
 * —— 绝不要把两份副本混在一起）。
 *
...
async function probeHttp(request, target, dispatcher) {
  ...
  if (verdict.kind !== 'http-error' || !headNeedsGetRetry(head.res.status)) return verdict
  const get = await attempt('GET')
  if (get.error !== undefined) return classifyTargetFailure(get.error)
  const retried = classifyProbeStatus(get.res.status)
  const sameStatus = get.res.status === head.res.status
  retried.note = sameStatus
    ? `HEAD 与 GET 均返回 ${get.res.status} — 该路径确实不可用。`
    : retried.ok === true
      ? `HEAD 返回 ${head.res.status}，GET 得到 ${get.res.status} — 以 GET 为准（网关常不给 HEAD 注册路由）。`
      : `HEAD 返回 ${head.res.status}，GET 得到 ${get.res.status} — 以 GET 为准。`
  return retried
}
...
/**
 * 按 web_fetch 的方式探测一个目标，**同时**强制走代理 + 强制直连两个通道，
 * 再把两侧结果综合成一条人话推荐（combineDualProbe）。
 *
 * 单侧探测无法判断被拒是谁干的（目标 or 代理?）——只有两侧对比才能得出
 * 方向性结论（直连通+代理失败 → 问题在代理一侧；代理通+直连失败 → 代理是
 * 唯一通路）。`channel` 保留兼容：'policy'/'proxy'/'direct' 仍是旧的单通道
 * 探测（HTTP API 的历史 useProxy 参数使用），默认 'dual' 走双通道并行。
 */
async function probeTarget(url, snapshot, channel = 'dual') {
  let parsed
  try {
    parsed = new URL(url)
  } catch {
    return { url, route: 'invalid', bypassed: false, channel,
      proxy: { ok: false, verdict: 'unusable', kind: 'invalid', short: 'URL 无效 — 需要 http(s)://主机/路径',
        why: '输入不是可解析的绝对 URL。', fix: '用完整形式，如 https://github.com/foo/bar。' },
      direct: { ok: false, verdict: 'unusable', kind: 'invalid', short: 'URL 无效 — 需要 http(s)://主机/路径',
        why: '输入不是可解析的绝对 URL。', fix: '用完整形式，如 https://github.com/foo/bar。' },
      verdict: 'both-bad', short: 'URL 无效', why: '输入不是可解析的绝对 URL。', fix: '用完整形式，如 https://github.com/foo/bar。' }
  }
  ...
  // 兼容旧单通道调用（HTTP API 历史 useProxy 参数）
  ...
  // 双通道并行：强制走代理 + 强制直连，各自独立探测（互不干扰）
  const proxyUrl = snapshot?.effective?.url ?? ''
  const proxyProbe = proxyUrl === ''
    ? { ok: false, verdict: 'unusable', kind: 'bad-proxy', short: '当前快照里没有可用的代理地址',
      why: '代理开关处于关闭（或 Windows 系统代理关闭），没有可强制走代理的对象。',
      fix: '打开代理，或在「代理配置」里修复地址/模式。' }
    : await forceChannelProbe(parsed, 'proxy', proxyUrl)
  const directProbe = await forceChannelProbe(parsed, 'direct', '')
  const combined = combineDualProbe(proxyProbe, directProbe)
  return {
    url, route: routeName, bypassed, channel: 'dual',
    proxy: proxyProbe, direct: directProbe,
    verdict: combined.verdict, short: combined.short,
    why: combined.why, fix: combined.fix,
    proxyBadge: combined.proxyBadge, directBadge: combined.directBadge,
  }
}
...
function apply(ctx, config) {
  ...
  // composition entry; every committed change re-syncs live. Registration goes
  // through the settings SERVICE — newer dsh-settings removed the
  // `installSettingsSection` export entirely, so importing it throws a
  // SyntaxError at boot and kills the whole profile (LESSONS §26).
  // register() + scope.get()/watch() exist on every version we target, and
  // update() recomputes registration.resolved, so scope.get() stays fresh.
  const settingsService = ctx.get('settings')
  if (settingsService !== undefined && typeof settingsService.register === 'function') {
    const proxyScope = settingsService.register(PROXY_NS, Config, { base: config })
    source = () => proxyScope.get()
    proxyScope.watch(() => {
      void requestSync().catch(() => {})
    })
  }
  // Without a settings service in this composition, `source` keeps the
  // composition default (initialized above) and the plugin still runs, read-only.
  ...
  ctx.tools.register(defineTool({
    ...
    output: {
      ...
      render: (_args, value) => {
        const badge = (b) => (b?.tone === 'ok' ? '🟢' : b?.tone === 'warn' ? '🟡' : '🔴') + ' ' + (b?.text ?? '未知')
        const lines = [
          `路由: ${value.route}`,
          `NO_PROXY 命中: ${value.bypassed}`,
          `走代理: ${badge(value.proxyBadge)}`,
          `直连:   ${badge(value.directBadge)}`,
          `结论: ${value.short ?? '未知'}`,
        ]
        if (value.why) lines.push(`可能原因: ${value.why}`)
        if (value.fix) lines.push(`挽救措施: ${value.fix}`)
    ...
    execute: async (args) => {
      ...
      result.hint = keepaliveHint(`${result.proxy?.short ?? ''} ${result.direct?.short ?? ''}`, snapshot?.effective?.url ?? '')
      return result
    },
    presentCall: (args) => ({
      card: 'generic',
        ...
        handler: async (req, res) => {
          ...
          try {
            ...
            if (is('/route', 'GET')) {
              ...
              // useProxy 保留兼容：'1'/'0' 走旧的单通道强制；无参数（新版面板）走双通道
              const useProxy = url.searchParams.get('useProxy')
              const channel = useProxy === '1' ? 'proxy' : useProxy === '0' ? 'direct' : 'dual'
              const result = await probeTarget(target, snapshot, channel)
              result.hint = keepaliveHint(
                `${result.proxy?.short ?? ''} ${result.direct?.short ?? ''}`.trim(),
                snapshot?.effective?.url ?? ''
              )
