...
function classifyTargetFailure(error) {
  ...
  if (code === 'EACCES' || code === 'ENETUNREACH' || code === 'EHOSTUNREACH' || code === 'ENOTFOUND' || code === 'ECONNREFUSED' || code === 'ETIMEDOUT') {
...
async function forceChannelProbe(url, channel, proxyUrl) {
  ...
  if (und === null) {
    return { ok: false, kind: 'unavailable', short: 'forced channel unavailable (undici could not be loaded)' }
  }
  let dispatcher
  try {
    ...
    return { ok: false, kind: 'bad-proxy', short: `invalid proxy address: ${String(error?.message ?? error)}` }
  }
  ...
  try {
    ...
    // 407 is an authentication failure, not "reachable": reporting it as ok
    // made the panel print 「可达（HTTP 407）」 and silenced the keepalive hint
    // (review fix, 2026-09-17).
    if (res.status === 407) {
      return { ok: false, kind: 'auth', short: 'HTTP 407 — the proxy asks for authentication (NTLM), which undici cannot negotiate' }
    }
    return { ok: true, status: res.status }
  } catch (error) {
    return classifyTargetFailure(error)
  } finally {
    clearTimeout(timer)
...
async function probeTarget(url, snapshot, channel = 'policy') {
  ...
  try {
    ...
    return { url, route: 'invalid', bypassed: false, probe: { ok: false, kind: 'invalid', short: 'invalid URL — expected http(s)://host/path' } }
  }
  ...
  if (channel === 'proxy' || channel === 'direct') {
    ...
    const probe = proxyUrl === '' && channel === 'proxy'
      ? { ok: false, kind: 'bad-proxy', short: 'no active proxy url in the current snapshot' }
  ...
  try {
    ...
    clearTimeout(timer)
    if (res.status === 407) {
      return { url, route: routeName, bypassed, probe: { ok: false, kind: 'auth', short: 'HTTP 407 — the proxy asks for authentication (NTLM), which undici cannot negotiate' } }
    }
...
function apply(ctx, config) {
  ...
  const sync = async () => {
    ...
    const effKey = JSON.stringify([active, effective.url ?? null, effective.noProxy ?? null, effective.reason ?? 'unknown'])
    ...
    try {
      const unregisterRoute = webServer.register({
        ...
        handler: async (req, res) => {
          ...
          try {
            if (is('/status', 'GET')) {
              // Read-only: serve the latest snapshot directly. The snapshot is
              // kept fresh by the config hook and the system-poll timer, so a
              // poller must NEVER trigger a full re-sync here — doing that made
              // every GUI poll (2×~4s) reinstall the global dispatcher, which
              // dragged the whole desktop down (LESSONS §17).
