...
/** Reusable per-channel probe result (a classifyProbeStatus / classifyTargetFailure
 *  output). Schema DSL has no $ref, so this inlines into PROXY_TEST_SCHEMA. */
...
/** Output schema of `proxy_test`. Dual-channel: forces both legs in parallel
 * (走代理 / 直连), then combines them into one human verdict + recommendation
 * (2026-09-18: single-side probes cannot tell WHO answered, so a strategy
 * block can only be inferred by comparing both sides — LESSONS §30). */
export const PROXY_TEST_SCHEMA = {
  ...
  properties: {
    ...
    proxy: PROBE_RESULT_SCHEMA,
    direct: PROBE_RESULT_SCHEMA,
