...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function ProxyManagerPanel(props) {
      ...
      /** Three-state headline for one probe verdict (usable / degraded / unusable). */
      function probeHeadline(probe) {
        if (!probe) return "未知";
        if (probe.verdict === "usable") return "可用" + (probe.status === undefined ? "" : "（HTTP " + probe.status + "）");
        if (probe.verdict === "degraded") return "降级：" + (probe.short || "未知");
        if (probe.verdict === "unusable") return "不可用：" + (probe.short || "未知");
        return probe.ok ? "可用" : (probe.short || "未知");
      }
      function runDiag() {
            ...
            ? react.createElement("div", { className: "dsxprx-diagout" },
              diag.error
                ...
                : [
                  ...
                  react.createElement("span", { key: "p" }, "连通探测： " + probeHeadline(diag.probe)),
                  diag.probe && diag.probe.why ? react.createElement("span", { key: "w", className: "dsxprx-diagwhy" }, "可能原因： " + diag.probe.why) : null,
                  diag.probe && diag.probe.fix ? react.createElement("span", { key: "f", className: "dsxprx-diagfix" }, "挽救措施： " + diag.probe.fix) : null,
                  diag.probe && diag.probe.note ? react.createElement("span", { key: "n", className: "dsxprx-diagnote" }, "备注： " + diag.probe.note) : null,
