 ...
 *
 * The registration id MUST be the PACKAGE name: dsh-client-modules stores the
 * factory under stripClientSuffix(id) and later looks it up by the graph row
 * id (= package name). Registering "dsh-proxy-pro/client" (the pre-scope name)
 * throws 'bundle ... loaded without registering "@yanglaofish/dsh-proxy-pro"
 * via __ModuleLoader__.load' (see docs/LESSONS.md §18).
 */
window.__ModuleLoader__.load({
  id: "@yanglaofish/dsh-proxy-pro",
  factory: (require) => {
    var module = { exports: {} };
    ...
    var css = [
      ...
      ".dsxprx-diagwhy{font-size:12px;line-height:17px;color:var(--dsw-alias-label-secondary)}",
      ".dsxprx-diagfix{font-size:12px;line-height:17px;color:var(--dsw-alias-label-primary)}",
      ".dsxprx-diagnote{font-size:11px;line-height:16px;color:var(--dsw-alias-label-caption)}",
      ".dsxprx-verdict{display:inline-flex;align-items:center;gap:6px;padding:2px 10px;border-radius:999px;font-weight:600;font-size:12px;line-height:18px}",
      ".dsxprx-verdict[data-tone=ok]{background:rgba(16,185,129,.12);color:var(--dsw-alias-state-success-primary,#10b981)}",
      ".dsxprx-verdict[data-tone=warn]{background:rgba(245,158,11,.14);color:#f59e0b}",
      ".dsxprx-verdict[data-tone=bad]{background:rgba(239,68,68,.12);color:var(--dsw-alias-state-error-primary,#ef4444)}",
      ".dsxprx-diagprobe{display:flex;align-items:center;gap:8px;flex-wrap:wrap}",
      ".dsxprx-diotag{flex:none;min-width:44px;font-size:12px;line-height:17px;color:var(--dsw-alias-label-caption)}",
      ".dsxprx-verdictlarge{font-size:13px;font-weight:600;line-height:19px;color:var(--dsw-alias-label-primary)}",
      ".dsxprx-diagverdict{display:flex;flex-direction:column;gap:3px;padding:6px 0 2px;border-top:1px dashed var(--dsw-alias-border-l1)}",
    ...
    function ProxyHeaderButton(props) {
      ...
      // Local optimistic value (2026-09-18): the capsule flips its visual
      // immediately on click instead of waiting for scope.set's round-trip
      // (the settings-page switch already behaves this way). null means no
      // unconfirmed value — the capsule mirrors settings.value.
      var optimistic = useState(null);
      var setOptimistic = optimistic[1];
      var config = settings.value || {};
      var valueEnabled = config.enabled === true;
      var shownEnabled = optimistic[0] !== null ? optimistic[0] : valueEnabled;
      var active = shownEnabled === true && (config.mode || "system") !== "none";
      var mode = config.mode || "system";
      var modeLabel = mode === "system" ? "system" : mode === "custom" ? "custom" : "none";
      // Once the host has committed the new value, drop the optimistic echo.
      useEffect(function () {
        if (optimistic[0] !== null && settings.ready && settings.value !== undefined && settings.value.enabled === optimistic[0]) {
          setOptimistic(null);
        }
      }, [optimistic[0], settings.value, settings.ready]);
      function toggle() {
        if (!settings.writable) return;
        var next = !shownEnabled;
        setOptimistic(next);
        scope.set("enabled", next).catch(function (error) {
          console.error("[dsh-proxy-pro] toggle failed:", error);
          // Roll back to the committed value on failure.
          setOptimistic(null);
        });
        // Toggle doubles as a manual "recheck now" even when the settings
        // panel is not mounted: pulling /status makes the host re-read the
        // system facts (idempotent — snapshot refresh unless the proxy really
        // changed). The settings hook already re-syncs on scope.set; this
        // pull is a visible, UI-independent follow-up so 关闭→打开 re-applies
        // and re-verifies the proxy from the header alone.
        fetchJson("/status");
      }
      var title = active
        ? "代理已开启（界面开关）· 点击关闭"
    ...
    function ProxyManagerPanel(props) {
      ...
      var effectiveNoProxy = status && status.noProxy
        ? status.noProxy
        : (value.noProxy || "").trim();
      // "含系统 ProxyOverride 并入" only makes sense in system mode with the
      // proxy ACTUALLY active: custom mode keeps its own list and a disabled/
      // unmatched state returns only the always-local entries — comparing raw
      // strings alone mislabeled those as a system merge (review fix, 2026-09-17).
      var systemMerged = modeNow === "system"
        && status !== null && status !== undefined && status.active === true && status.noProxy
      ...
      // 诊断双通道并行（走代理 + 直连），由 host 一次给出两侧徽章和推荐，
      // 不再让用户手动勾选通道（2026-09-18：单侧探针分不清被拒者是谁）
      function runDiag() {
      ...
      /** 通道徽章：优先用 host 的 proxyBadge/directBadge（人话+色），缺失时回退旧判定。 */
      function channelBadge(name) {
        var b = diag && typeof diag[name] === "object" && diag[name] !== null ? diag[name] : null;
        if (b && b.text && b.tone) return b;
        var fallback = name === "proxyBadge" ? diag?.proxy : diag?.direct;
        if (!fallback) return { text: "未知", tone: "warn" };
        if (fallback.verdict === "usable") return { text: "链路通 · 目标正常响应", tone: "ok" };
        if (fallback.verdict === "degraded") return { text: "链路通 · 这次请求被目标拒绝", tone: "warn" };
        // 5xx：拿到了 HTTP 应答，链路通但服务端/网关报错（严重性误导修复：不算"链路不通"）
        if (fallback.kind === "gateway" || fallback.kind === "server-error") return { text: "链路通 · 目标/网关报错", tone: "warn" };
        if (fallback.unusable === true || fallback.verdict === "unusable") return { text: "链路不通 · 网络/代理问题", tone: "bad" };
        return fallback.ok ? { text: "链路通", tone: "ok" } : { text: "链路不通", tone: "bad" };
      }
      ...
      return react.createElement("div", { className: "dsxprx", "data-proxy-settings": true },
        ...
        react.createElement("div", { className: "dsxprx-diag" },
          ...
          react.createElement("span", { className: "dsxprx-hint" }, "同时测「走代理」与「直连」两条通道并给出推荐，可判断该域名该不该加进 NO_PROXY。"),
          diag !== null
            ? react.createElement("div", { className: "dsxprx-diagout" },
              diag.error
                ...
                : [
                  ...
                  react.createElement("span", { key: "codes" }, "状态码: 走代理 " + ((diag.proxy && diag.proxy.status) || "—") + " · 直连 " + ((diag.direct && diag.direct.status) || "—")),
                  react.createElement("span", { key: "px", className: "dsxprx-diagprobe" },
                    react.createElement("span", { className: "dsxprx-diotag" }, "走代理"),
                    react.createElement("span", { className: "dsxprx-verdict", "data-tone": channelBadge("proxyBadge").tone }, channelBadge("proxyBadge").text)
                  ),
                  react.createElement("span", { key: "dx", className: "dsxprx-diagprobe" },
                    react.createElement("span", { className: "dsxprx-diotag" }, "直连"),
                    react.createElement("span", { className: "dsxprx-verdict", "data-tone": channelBadge("directBadge").tone }, channelBadge("directBadge").text)
                  ),
                  diag.verdict ? react.createElement("span", { key: "v", className: "dsxprx-diagverdict" },
                    react.createElement("span", { className: "dsxprx-verdictlarge" }, "推荐： " + (diag.short || "未知")),
                    diag.why ? react.createElement("span", { className: "dsxprx-diagwhy" }, "可能原因： " + diag.why) : null,
                    diag.fix ? react.createElement("span", { className: "dsxprx-diagfix" }, "挽救措施： " + diag.fix) : null
                  ) : null,
                  diag.proxy && diag.proxy.note ? react.createElement("span", { key: "n", className: "dsxprx-diagnote" }, "备注： " + diag.proxy.note) : null,
