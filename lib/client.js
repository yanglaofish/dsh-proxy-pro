...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var css = [
      ...
      ".dsxprx-switchrow{display:flex;align-items:center;justify-content:space-between;gap:10px;padding:10px 12px;border:1px solid var(--dsw-alias-border-l1);border-radius:10px;background:var(--dsw-specific-tip)}",
      ".dsxprx-switchlabel{font-size:13px;font-weight:600;color:var(--dsw-alias-label-primary)}",
    ...
    function ProxyManagerPanel(props) {
      ...
      if (cfgActive) {
        if (status !== null && status !== undefined) {
          subLine = status.active === true
            ? (status.source === "system" ? "跟随 Windows 系统代理（自动刷新）" : "自定义地址" + (status.url ? " · " + status.url : ""))
      ...
      var effectiveNoProxy = status && status.noProxy
        ? status.noProxy
        : (value.noProxy || "").trim();
      var systemMerged = status !== null && status !== undefined && status.noProxy
      ...
      return react.createElement("div", { className: "dsxprx", "data-proxy-settings": true },
        ...
        // --- enable switch (independent row) ---
        react.createElement("div", { className: "dsxprx-switchrow" },
          react.createElement("span", { className: "dsxprx-switchlabel" }, "启用代理"),
