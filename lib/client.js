...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** Fetch the host status snapshot on mount, whenever `refreshKey` changes
     *  (config edits / mode switches), and every 30s while the panel is open.
     *  /status is a pure read-only snapshot in the host — it never re-syncs
     *  and never reinstalls the dispatcher — so a slow poll is cheap and only
     *  surfaces system-proxy changes the host's own 30s poll already applied
     *  (review fix, 2026-09-17; §17: read ≠ rebuild). */
