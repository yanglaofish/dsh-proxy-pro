...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** Poll the host status API every `intervalMs`; `{ loading, status, error }`. */
    function useRuntimeStatus(intervalMs) {
    ...
    function ProxyHeaderButton(props) {
      ...
      var runtime = useRuntimeStatus(4000);
      var busy = useState(false);
      var setBusy = busy[1];
      var config = settings.value || {};
      var active = runtime.status !== null && runtime.status !== undefined
        ? runtime.status.active === true
        : config.enabled === true && config.mode !== "none";
      var enabled = runtime.status !== null && runtime.status !== undefined
        ? runtime.status.enabled === true
        : config.enabled === true;
      var mode = (runtime.status !== null && runtime.status !== undefined ? runtime.status.mode : config.mode) || "system";
      ...
      var title = active
        ? "代理已开启（" + (runtime.status && runtime.status.url || "") + "）· 点击关闭"
        : "代理已关闭" + (runtime.status && runtime.status.reason ? " · " + runtime.status.reason : "") + " · 点击开启";
    ...
    function ProxyManagerPanel(props) {
      var scope = props.scope;
      var settings = useProxySnapshot(scope);
      var runtime = useRuntimeStatus(3000);
