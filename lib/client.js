...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** Fetch the host status snapshot on mount and again whenever `refreshKey`
     *  changes (config edits / mode switches). No timed polling: the key only
     *  moves when the user changes the configuration (§17: read ≠ poll). */
